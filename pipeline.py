...
def get_model_matrix(mesh: Mesh) -> glm.mat4:
    # rotation_x = glm.mat4.from_x_rotation(mesh.rotation.x)
    # rotation_y = glm.mat4.from_y_rotation(mesh.rotation.y)
    # rotation_z = glm.mat4.from_z_rotation(mesh.rotation.z)
    # scale_matrix = glm.mat4.from_scale(mesh.scale)
...
def world_to_screen(vertex: glm.vec4, width: float, height: float) -> glm.vec2:
...
def draw_face(bitmap: Image.Image, face: Face, mvp: glm.mat4, width: float, height: float, fill: tuple) -> NoReturn:
    ...
    p0: glm.vec2 = world_to_screen(p0, width, height)
    p1: glm.vec2 = world_to_screen(p1, width, height)
    p2: glm.vec2 = world_to_screen(p2, width, height)

    draw.line([*p0.xy, *p1.xy], fill=fill)
    draw.line([*p1.xy, *p2.xy], fill=fill)
    draw.line([*p2.xy, *p0.xy], fill=fill)
    draw.polygon([*p0.xy, *p1.xy, *p2.xy], fill=fill)
